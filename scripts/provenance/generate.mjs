...
import { buildRunsRows } from './runs.mjs';

const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(SCRIPT_DIR, '..', '..');
const REPORTS_DIR = path.join(REPO_ROOT, 'reports');
export const OUTPUT_PATH = path.join(REPO_ROOT, 'src', 'content', 'provenance.generated.json');
// The runs artifact (`docs/reports-surface.md` §3.2) — a SEPARATE committed
// file from `OUTPUT_PATH` above, generated by the same script for the same
// reason `provenance.generated.json` is (§5.2's design: "removes the
// possibility of drift rather than policing it"), but by a git-free code
// path — see `generateRunsArtifact` below.
export const RUNS_OUTPUT_PATH = path.join(REPO_ROOT, 'src', 'content', 'runs.generated.json');
...
async function loadContentModules(repoRoot) {
  ...
  try {
    ...
    return {
      ...
      RunsArtifactSchema: schemaModule.RunsArtifactSchema,
      RUN_KIND_BY_H1_PREFIX: schemaModule.RUN_KIND_BY_H1_PREFIX,
    };
  } finally {
    await server.close();
  }
...
/**
 * Builds the runs artifact (`docs/reports-surface.md` §3.2) — one row per
 * file in `reports/`. Unlike `generateProvenance` above, this is entirely
 * GIT-FREE: every field (`runId`, `reportPath`, `date`, `title`, `kind`) is
 * derived from the filename and the file's own first H1, never from git
 * history. That means it needs none of `generateProvenance`'s shallow-clone
 * handling and can always run, even in the Vercel-deploy fallback path where
 * `generateProvenance` itself can't read full history (see `main` below).
 *
 * @param {object} [options]
 * @param {string} [options.repoRoot]
 * @param {string} [options.reportsDir]
 * @param {() => Promise<{RunsArtifactSchema: import('zod').ZodType, RUN_KIND_BY_H1_PREFIX: Record<string,string>}>} [options.loadModules]
 */
export async function generateRunsArtifact({ repoRoot = REPO_ROOT, reportsDir = REPORTS_DIR, loadModules = loadContentModules } = {}) {
  const { RunsArtifactSchema, RUN_KIND_BY_H1_PREFIX } = await loadModules(repoRoot);
  const files = readReportFiles(reportsDir, repoRoot);
  // §3.2 failure table: no-date / no-H1 both throw `RunsArtifactValidationError`
  // (propagates to the caller) naming every offending file at once.
  const rows = buildRunsRows({ files, kindByH1Prefix: RUN_KIND_BY_H1_PREFIX });
  // Defense in depth, mirroring `generateProvenance`'s re-validation of each
  // record against `ProvenanceRecordSchema` before returning it.
  return RunsArtifactSchema.parse(rows);
}
...
async function main() {
  ...
  // Runs artifact: deliberately its OWN try/catch, independent of the
  // provenance outcome above — a git-availability problem in one has no
  // bearing on the other (this one never touches git at all), and a content
  // defect in one (a bad `yaml provenance` block vs. a malformed report
  // filename/H1) should never mask the other. Both failures, if both occur,
  // are surfaced in the same run rather than the second being hidden behind
  // an early return.
  try {
    const rows = await generateRunsArtifact();
    mkdirSync(path.dirname(RUNS_OUTPUT_PATH), { recursive: true });
    writeFileSync(RUNS_OUTPUT_PATH, `${JSON.stringify(rows, null, 2)}\n`, 'utf8');
    console.log(`[runs] wrote ${rows.length} row${rows.length === 1 ? '' : 's'} -> ${path.relative(REPO_ROOT, RUNS_OUTPUT_PATH)}`);
  } catch (error) {
    console.error('[runs] generation failed:\n');
    console.error(error?.message ?? String(error));
    ok = false;
  }
