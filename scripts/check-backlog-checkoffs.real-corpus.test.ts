...
describe('checkBacklogCheckoffs — real corpus (this repo\'s actual reports/BACKLOG.md + real `gh pr list`)', () => {
  itRealCorpus('is CLEAN, and every referencedButOpen note is well-formed (no specific branch pinned — see comment)', () => {
    ...
    // BACKLOG.md argues is worse than no gate at all.
    //
    // The ported fix pinned the known `team/2026-08-04-runs-api` epic with
    // `toContainEqual(...)` instead — and THAT went red the same day, for a
    // reason worth recording here because it is a property of the gate rather
    // than of this test. `classify()` returns 'checked' on the FIRST `[x]`
    // block whose text merely `includes(branch)`. The 2026-08-11 run checked
    // off an unrelated item whose prose happens to *mention*
    ...
    // and never was — the synthetic "THE PR #100 FALSIFICATION FIXTURE" test in
